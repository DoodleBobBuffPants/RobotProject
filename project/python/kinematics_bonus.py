...
def rk4(current_pose, t, dt):
  ...
  tp = t - np.floor(t)
  K1YAW = dt * np.cos(np.floor(t)+tp)
  K2YAW = dt * np.cos(np.floor(t + dt/2)+tp)
  K3YAW = dt * np.cos(np.floor(t + dt/2)+tp)
  K4YAW = dt * np.cos(np.floor(t + dt)+tp)
  next_pose[YAW] = current_pose[YAW] + ((1. / 6.) * (K1YAW + 2*K2YAW + 2*K3YAW + K4YAW))

  K1X = dt * 0.25 * np.cos(current_pose[YAW])
  K2X = dt * 0.25 * np.cos(current_pose[YAW] + K1YAW/2)
  K3X = dt * 0.25 * np.cos(current_pose[YAW] + K2YAW/2)
  K4X = dt * 0.25 * np.cos(current_pose[YAW] + K3YAW)
  next_pose[X] = current_pose[X] + ((1. / 6.) * (K1X + 2*K2X + 2*K3X + K4X))

  K1Y = dt * 0.25 * np.sin(current_pose[YAW])
  K2Y = dt * 0.25 * np.sin(current_pose[YAW] + K1YAW/2)
  K3Y = dt * 0.25 * np.sin(current_pose[YAW] + K2YAW/2)
  K4Y = dt * 0.25 * np.sin(current_pose[YAW] + K3YAW)
  next_pose[Y] = current_pose[Y] + ((1. / 6.) * (K1Y + 2*K2Y + 2*K3Y + K4Y))
