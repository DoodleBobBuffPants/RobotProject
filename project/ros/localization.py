...
class Particle(object):
  ...
  def __init__(self):
    ...
    self._pose[X] = random.uniform(-2,2)
    self._pose[Y] = random.uniform(-2,2)
    self._pose[YAW] = random.uniform(0, 2*np.pi)
  ...
  def is_valid(self):
    ...
    x = self._pose[X]
    y = self._pose[Y]
    if -2 <= x <= 2 and -2 <= y<= 2 and np.sqrt((x-0.3)**2 + (y-0.2)**2) > 0.3:
        return True
  ...
  def move(self, delta_pose):
    ...
    du = np.random.normal(delta_pose[X], (0.1*np.abs(delta_pose[X]) + 0.0001))
    dw = np.random.normal(delta_pose[YAW], (0.1*np.abs(delta_pose[YAW]) + 0.0001))
    self._pose[X] = self._pose[X] + du*np.cos(self._pose[YAW])
    self._pose[Y] = self._pose[Y] + du*np.sin(self._pose[YAW])
    self._pose[YAW] = self._pose[YAW] + dw
  ...
  def compute_weight(self, front, front_left, front_right, left, right):
    ...
    def prob_normal(x, xbar):
    	return np.exp((-((x-xbar)**2))/(2.*variance)) / (sigma*np.sqrt(2.*np.pi))

    if not self.is_valid():
        self._weight = 0.1
    else:
        d_front = self.ray_trace(0.)
        d_front_left = self.ray_trace(np.pi/4.)
        d_front_right = self.ray_trace(-np.pi/4.)
        d_left = self.ray_trace(np.pi/2.)
        d_right = self.ray_trace(-np.pi/2.)

        nw = 1.
        if front != float('inf') and d_front != float('inf'):
        	nw = nw * prob_normal(d_front, front)
        if front_left != float('inf') and d_front_left != float('inf'):
        	nw = nw * prob_normal(d_front_left, front_left)
        if front_right != float('inf') and d_front_right != float('inf'):
        	nw = nw * prob_normal(d_front_right, front_right)
        if left != float('inf') and d_left != float('inf'):
        	nw = nw * prob_normal(d_left, left)
        if right != float('inf') and d_right != float('inf'):
        	nw = nw * prob_normal(d_right, right)
