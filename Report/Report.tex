...
Visually our solution in both the centralized and decentralized case achieves formation control as seen by the sequence of images in Fig. X and Fig. Y. In order to quantify our results, we evaluate the performance using the error to the desired position for each robot over time, calculated as:
\[err = \lvert F_i - P_{R_i}\rvert\]
This acts as a measure for how out of formation robot $R_i$ is at each point in time.

% TODOS:
% 1) keep formation, successfully change formation, split-and-merge, tight corners?
...
% 3) edit error plots
% 4) fill out tables (change them), exploration (better with split-and-merge)
...
% TODO: plots
% TODO: rigorous evaluation i.e. mention specific points on the curves
