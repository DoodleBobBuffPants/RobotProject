...
We define the position error for robot $R_i$ as $|F_{i} - P_{R_i}|$. Tables \ref{tab:results_corridor} and \ref{tab:results_square} show the performance of all formations in two arenas (TODO figures for two arenas)

From Fig 4. \& 5. we can see the centralised approach achieves a lower average error. This is due to robots regaining formation faster and deviating less when splitting around obstacles. We can further see in tables \ref{tab:results_corridor} and \ref{tab:results_square} the centralized solution achieves lower position errors and on most runs faster time to the goal. The larger peaks in the curves show split and merge behaviour. While all robots in the centralised approach merge after a split within 15s, larger times, as much as 25s were seen in the decentralised system. 

As expected, the column formation takes the shortest time to reach the goal, and has the lowest position error. However, it offers no extra exploration of the arena over having a single robot (since followers all follow the same path as the leader). The line formation offers the best exploration of the arena, but in general the diamond and wedge formations take less time and have less error. E.g the diamond takes 

Both systems are able to successfully split and merge around obstacles, navigate tight corners and switch formation to handle corridors.
...
\textit{diamond} & 76                     & 0.40               & 101                    & 1.05 \\     
                \hline
\end{tabular}
\caption{System results in the corridor arena}
...
\hline
\end{tabular}
\caption{System results in the square arena}
