...
\section{RESULTS}
We conducted experiments in multiple arenas, presenting results below for the square (Fig \ref{fig:square}) and corridor (Fig \ref{fig:corridorworld}) arenas, to evaluate the performance of our approach. The square arena features multiple obstacles, while the corridor challenges the robot through narrow passages. 

Both systems successfully split and merge around obstacles, navigate tight corners and switch formation to handle corridors. In the video we further demonstrate robots splitting/merging around an obstacle added at runtime. Fig  \ref{fig:split_merge} shows diamond splitting around narrow obstacles in the square arena, and Fig \ref{fig:switch_formation} shows the wedge switching to a column in the corridor arena.
...
We define the position error for robot $R_i$ as $|F_{i} - P_{R_i}|$. This is then averaged over time across all the follower robots. Tables \ref{tab:results_corridor} and \ref{tab:results_square} show the performance of all formations in two arenas.

From Fig \ref{fig:centralized} \& \ref{fig:decentralized} we can see the centralised approach achieves a lower average error. This is due to robots regaining formation faster and deviating less when splitting around obstacles. We can further see in tables \ref{tab:results_corridor} and \ref{tab:results_square} the centralized solution achieves lower position errors and on most runs faster time to the goal. The larger peaks in the curves show split and merge behaviour. While all robots in the centralised approach merge after a split within 15s, larger times, as much as 25s were seen in the decentralised system. The centralized system is faster, with lower position errors, e.g for diamond centralised achieves 63 seconds over 75 and an error of 0.27 over 0.62, in the corridor arena. 

As expected, the column formation takes the shortest time to reach the goal, and has the lowest position error. However, it offers no extra exploration of the arena over having a single robot (since followers all follow the same path as the leader). The line formation offers the best exploration of the arena, but in general the diamond and wedge formations take less time and have less error. E.g the wedge in the square arena takes 64 seconds (over 68 for line) and has error 0.57 (over 0.64 for line).
...
\end{figure}
% todo add units to this. labels for robots isnt right
\begin{figure}[tb]
    \centering
...
In this paper we have shown an implementation of both centralized and decentralized multi-robot formation control with varying levels of success. Within simulation each approach visibly maintains formation whilst the robots traverse an arena and avoid obstacles. The centralized approach performs better as shown in the evaluation results, whereas the decentralized approach is not as robust. Each approach can generalize to any new arena too.
...
\item Ben Philps - Multi-robot obstacle avoidance for centralised, decentralised systems experimenting with different approaches. Multi-robot avoidance. End-to-end testing, ensuring general solution. Results collection, footage.
