...
def get_relative_position(absolute_pose, absolute_position):
  ...
  vec = absolute_position - np.array([absolute_pose[X], absolute_pose[Y]])
  theta = np.arctan(vec[Y]/vec[X]) - absolute_pose[YAW]

  return np.array([vec[X]*np.cos(theta) - vec[Y]*np.sin(theta), vec[X]*np.sin(theta) + vec[Y]*np.cos(theta)])
