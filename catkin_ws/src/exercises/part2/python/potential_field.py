...
CYLINDER_RADIUS = .3
CYLINDER_POSITION_2 = np.array([.0, .5], dtype=np.float32)
CYLINDER_RADIUS_2 = .3
...
def get_velocity_to_reach_goal(position, goal_position):
  ...
  return cap(2*(goal_position-position), MAX_SPEED)
...
def get_velocity_to_avoid_obstacles(position, obstacle_positions, obstacle_radii):
  ...
  for o_pos, o_rad in zip(obstacle_positions, obstacle_radii):
  	if np.linalg.norm(position-o_pos) <= o_rad + 0.1:
  		vec = MAX_SPEED*normalize(position-o_pos)
  		v += 20*np.array([vec[1], -vec[0]])
...
def get_velocity(position, mode='all'):
  ...
  if mode in ('obstacle', 'all'):
    v_avoid = get_velocity_to_avoid_obstacles(
      position,
      [CYLINDER_POSITION, CYLINDER_POSITION_2],
      [CYLINDER_RADIUS, CYLINDER_RADIUS_2])
  else:
...
if __name__ == '__main__':
  ...
  ax.add_artist(plt.Circle(CYLINDER_POSITION_2, CYLINDER_RADIUS_2, color='gray'))
